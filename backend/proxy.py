...
class WebRequestInterceptor:
    ...
    def request(self, flow: mitmproxy.http.HTTPFlow):
        ...
        if request.host not in self.scope:
            if flow.intercepted:
                flow.resume()
            self.send_request2_to_http_trafic_tab(request)
            # self.send_request2_to_http_trafic_tab(flow.response)
    ...
    async def handle_kill_request(self, reader, writer):
        ...
        if data:
            ...
            url = "https://google.com"  # URL do wysłania żądania
            content = ""  # Treść żądania (dla POST/PUT)
            ...
            # Tworzenie instancji Request za pomocą klasy `make`
    ...
    def send_request2_to_http_trafic_tab(self, request):
        """
        Serializes request and sends it to GUI HTTP traffic tab.
        """
        try:
